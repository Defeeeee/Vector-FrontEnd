...
import { login, getGoogleLoginUrl, getAppleLoginUrl, setSession } from "@/actions/auth";
...
function LoginContent() {
  ...
  async function handleAppleLogin() {
    ...
    setError(null);
    try {
      const result = await getAppleLoginUrl();
      if (result.error) {
        setError(result.error);
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                >
                    {isApplePending ? (
                        ...
                        <>
                            <svg className="w-4 h-4" viewBox="0 0 24 24">
                                <path fill="currentColor" d="M17.05 20.28c-.96.95-2.21 1.72-3.72 1.72-1.47 0-2.33-.89-3.73-.89s-2.45.89-3.71.89c-1.55 0-2.99-.92-3.95-2.43C.05 16.63-.51 12.39 1.43 9.03c.96-1.68 2.69-2.75 4.59-2.75 1.44 0 2.41.87 3.54.87s2.17-.87 3.74-.87c1.37 0 2.7.54 3.61 1.54-3.13 1.25-2.6 5.61.64 6.94-.73 1.76-1.58 3.51-2.9 5.52zM12.03 5.98c-.08-2.69 2.21-4.91 4.55-5.18.25 2.87-2.34 5.34-4.55 5.18z" />
                            </svg>
                            <span>Apple</span>
                        </>
                    )}
