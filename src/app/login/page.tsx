    ...
    <div className="flex flex-col items-center justify-center min-h-screen w-full max-w-md mx-auto px-6 py-12 relative z-10">
      ...
      <AnimatePresence>
        {expired && (
          ...
          >
            <motion.div 
              className="bg-white border border-zinc-200 rounded-[2.5rem] p-10 max-w-sm w-full text-center space-y-8 shadow-cal"
            >
              <div className="w-16 h-16 bg-red-50 text-red-600 rounded-full flex items-center justify-center mx-auto">
                <Lock className="w-8 h-8" />
              </div>
              <div className="space-y-2">
                <h2 className="text-2xl font-bold font-space-grotesk text-zinc-900 uppercase tracking-tighter">Sesión Expirada</h2>
              ...
              <button 
                ...
                className="w-full bg-zinc-900 text-white font-bold text-[10px] uppercase tracking-[0.2em] py-5 rounded-2xl shadow-cal-highlight hover:opacity-90"
              >
                Entendido
              </button>
            </motion.div>
      ...
      >
        <Link href="/" className="w-16 h-16 bg-zinc-900 text-white rounded-2xl flex items-center justify-center shadow-lg">
          <Compass className="w-8 h-8" strokeWidth={1.5} />
        </Link>
        <div className="text-center space-y-1">
          <h1 className="text-4xl font-space-grotesk font-bold tracking-tighter uppercase tracking-[0.1em] text-zinc-900">Vector</h1>
      ...
      >
        <form action={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2 group">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  required
                  className="w-full bg-transparent border border-zinc-200 rounded-2xl py-5 pl-14 pr-6 outline-none focus:border-zinc-400 transition-all text-sm font-bold tracking-wide placeholder:text-zinc-400 uppercase text-zinc-900"
                />
              </div>
            </div>
          ...
          <motion.button
            ...
            className="w-full bg-zinc-900 text-white font-bold text-xs uppercase tracking-[0.2em] py-5 rounded-2xl shadow-cal-highlight transition-all disabled:opacity-50 mt-2"
          >
            {isPending ? (
        ...
        <div className="relative py-2">
          ...
          <div className="relative flex justify-center text-[8px] font-bold uppercase tracking-widest">
            <span className="bg-white px-4 text-zinc-400">O continuar con</span>
          </div>
        </div>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.99 }}
          disabled={isPending || isGooglePending}
          className="w-full bg-zinc-50 border border-zinc-200 text-zinc-900 font-bold text-xs uppercase tracking-[0.2em] py-5 rounded-2xl transition-all hover:bg-zinc-100 disabled:opacity-50 flex items-center justify-center space-x-3 shadow-sm"
        >
          {isGooglePending ? (
        ...
        <div className="flex flex-col items-center space-y-6 pt-4 border-t border-zinc-100">
          <Link href="/recover" className="text-[10px] font-bold text-zinc-500 hover:text-zinc-900 transition-colors uppercase tracking-widest">
            ¿Olvidaste tu contraseña?
          </Link>
          
          <div className="flex items-center space-x-2 text-[10px] font-bold uppercase tracking-widest">
            <span className="text-zinc-500">¿Nuevo piloto?</span>
            <Link href="/register" className="text-zinc-900 hover:text-zinc-600 transition-colors border-b border-zinc-200 pb-0.5">
              Crear Cuenta
            </Link>
          </div>
        </div>
      ...
      {/* Version Tag */}
      <div className="absolute bottom-8 text-[10px] font-bold text-zinc-300 uppercase tracking-[0.5em]">
        Vector v1.0.0
      </div>
    </div>
  );
...
export default function Home() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-zinc-900 font-space-grotesk font-bold uppercase tracking-widest animate-pulse">Cargando Vector...</div>}>
      <LoginContent />
    </Suspense>
  );
}
