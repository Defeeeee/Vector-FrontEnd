...
import { Compass, Bot, User, ArrowLeft, Printer, TrendingUp, Award, Share2, Eye } from "lucide-react";
...
const CHAT_SIMULATIONS: Record<"hours" | "cma" | "route" | "goal", ChatSimMessage[]> = {
  hours: [
    ...
    { role: "assistant", content: "Analizando tu bitácora... 📊 Tenés un total de **485.5 horas de vuelo**, distribuidas principalmente en tu aeronave LV-XYZ (320.0h) y LV-ABC (165.5h)." }
  ],
  cma: [
...
export default function PromoFlyerPage() {
  ...
  const [viewMode, setViewMode] = useState<"flyer" | "linkedin">("linkedin");
  ...
  // Dynamic QR code API: matches background to prevent white block around it
  // Light card bg: bg-zinc-50 -> #f9fafb. Dark card bg: bg-[#111111]/[0.02] (effectively very dark gray or black #0c0c0c)
  ...
  return (
    <div className="min-h-screen bg-zinc-100 dark:bg-black text-zinc-900 dark:text-white transition-colors duration-300 flex flex-col items-center p-4 md:p-8 print:p-0 print:bg-white print:text-black">
      ...
      <header className="w-full max-w-5xl flex flex-col sm:flex-row items-center justify-between mb-8 pb-6 border-b border-zinc-200 dark:border-white/10 relative z-20 gap-4 print:hidden">
        <Link href="/" className="flex items-center space-x-2 text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white transition-colors text-xs font-bold uppercase tracking-widest self-start sm:self-auto">
          <ArrowLeft className="w-4 h-4" />
          <span>Volver al inicio</span>
        </Link>

        {/* View Mode Toggle */}
        <div className="flex bg-zinc-200 dark:bg-white/5 p-1 rounded-2xl border border-zinc-300/50 dark:border-white/10">
          <button
      ...
      <div className="w-full flex justify-center items-center overflow-x-auto print:overflow-visible py-4 print:py-0">
        ...
        <div
          id="flyer-container"
          className={`bg-white dark:bg-[#0c0c0c] border border-zinc-200 dark:border-white/10 rounded-[2.5rem] p-8 md:p-10 shadow-cal dark:shadow-none relative overflow-hidden transition-colors flex flex-col justify-between shrink-0 print:border-0 print:shadow-none print:p-0 print:bg-white print:text-black ${
            viewMode === "linkedin"
              ? "w-[1200px] h-[630px] aspect-[1200/630]"
              : "w-full max-w-[900px] min-h-[1150px]"
          }`}
        >
          ...
          {/* Dynamic Content depending on view mode */}
          {viewMode === "linkedin" ? (
            /* ======================================================== */
            /* LINKEDIN LANDSCAPE LAYOUT                                */
            /* ======================================================== */
            <div className="grid grid-cols-12 gap-6 flex-1 items-stretch my-2">
              
              {/* Left Column (Logo, title description & QR) */}
              <div className="col-span-4 flex flex-col justify-between bg-zinc-50 dark:bg-white/[0.02] border border-zinc-200 dark:border-white/5 rounded-3xl p-6 print:bg-zinc-50 print:border-zinc-200">
                ...
                {/* QR Code integrated with card color */}
              ...
              {/* Center Column (Phone Chat Simulator) */}
              <div className="col-span-4 flex flex-col justify-between bg-zinc-50 dark:bg-white/[0.02] border border-zinc-200 dark:border-white/5 rounded-3xl p-5 relative print:bg-zinc-50 print:border-zinc-200">
              ...
              {/* Right Column (Triggers & Progress Metric) */}
              <div className="col-span-4 flex flex-col justify-between gap-4">
                
                {/* Simulated triggers selector */}
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-10 flex-1 items-stretch">
                ...
                <div className="md:col-span-3 flex flex-col justify-between gap-6">
                  ...
                  {/* QR code integrated with background */}
