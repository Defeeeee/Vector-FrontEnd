...
import { CloudRain, Search, RefreshCw, ShieldAlert, BookOpen, Copy, Check, Compass, Play, MapPin, AlertCircle, ArrowRight, Wind, Thermometer, AlertTriangle, ChevronDown } from "lucide-react";
...
export default function RouteWeatherClient({ profile, flights }: RouteWeatherClientProps) {
  ...
  const [expandedStrip, setExpandedStrip] = useState<Record<string, boolean>>({});
  ...
  const toggleStrip = (key: string) => {
    setExpandedStrip((prev) => ({ ...prev, [key]: !prev[key] }));
  };

  // Weather rules config
  const categoryConfig: Record<string, { bg: string; text: string; dot: string; edge: string; label: string; severity: number }> = {
    VFR: { bg: "bg-emerald-500/10 border-emerald-500/20 dark:bg-emerald-500/5 dark:border-emerald-500/10", text: "text-emerald-600 dark:text-emerald-400", dot: "bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]", edge: "border-l-emerald-500", label: "VFR", severity: 0 },
    MVFR: { bg: "bg-blue-500/10 border-blue-500/20 dark:bg-blue-500/5 dark:border-blue-500/10", text: "text-blue-600 dark:text-blue-400", dot: "bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]", edge: "border-l-blue-500", label: "MVFR", severity: 1 },
    IFR: { bg: "bg-red-500/10 border-red-500/20 dark:bg-red-500/5 dark:border-red-500/10", text: "text-red-600 dark:text-red-400", dot: "bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]", edge: "border-l-red-500", label: "IFR", severity: 2 },
    LIFR: { bg: "bg-fuchsia-500/10 border-fuchsia-500/20 dark:bg-fuchsia-500/5 dark:border-fuchsia-500/10", text: "text-fuchsia-600 dark:text-fuchsia-400", dot: "bg-fuchsia-500 shadow-[0_0_10px_rgba(217,70,239,0.5)]", edge: "border-l-fuchsia-500", label: "LIFR", severity: 3 },
    UNK: { bg: "bg-zinc-100 border-zinc-200 dark:bg-white/5 dark:border-white/5", text: "text-zinc-500 dark:text-zinc-400", dot: "bg-zinc-500", edge: "border-l-zinc-300 dark:border-l-zinc-700", label: "UNK", severity: 0 }
  };
  ...
  return (
    <div className="space-y-8 pb-24">
      ...
      {/* Title Header */}
      <PageHeader eyebrow="Servicios de navegación" title="Planificador de Ruta">
        <p className="text-sm text-zinc-500 dark:text-zinc-400 max-w-2xl pt-1">
          Ingresá la ruta completa de tu vuelo para obtener un análisis meteorológico tramo por tramo del estado del METAR y TAF de cada aeródromo, incluyendo datos del registro oficial MADHEL de la ANAC.
        </p>
      </PageHeader>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#111111] border border-zinc-200 dark:border-white/10 rounded-[2rem] p-6 md:p-8 shadow-cal dark:shadow-none space-y-6">
          <div className="space-y-1">
            <h3 className="text-base font-bold text-zinc-900 dark:text-white">Planificar nuevo trayecto</h3>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4 items-stretch">
            <div className="flex-1 bg-zinc-50 dark:bg-white/5 border border-zinc-200 dark:border-white/10 rounded-2xl px-4 py-3.5 focus-within:ring-1 focus-within:ring-zinc-400 dark:focus-within:ring-white/20 transition-all flex items-center space-x-3">
              ...
              <input 
                ...
                className="bg-transparent text-sm font-semibold uppercase w-full outline-none text-zinc-900 dark:text-white placeholder:text-zinc-400 dark:placeholder:text-zinc-500"
              />
            </div>
            <button 
              type="submit" 
              className="bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 hover:bg-zinc-800 dark:hover:bg-zinc-100 font-semibold text-sm px-8 py-4 rounded-2xl transition-all active:scale-[0.98]"
            >
              Calcular ruta
            </button>
          </form>
          ...
          <div className="pt-2">
            <p className="text-xs font-medium text-zinc-400 mb-3">Ruta activa seleccionada</p>
        ...
        <div className="lg:col-span-1 bg-white dark:bg-[#111111] border border-zinc-200 dark:border-white/10 rounded-[2rem] p-6 md:p-8 shadow-cal dark:shadow-none space-y-6 flex flex-col justify-between h-full min-h-[260px]">
          <div className="space-y-1">
            <h3 className="text-base font-bold text-zinc-900 dark:text-white">Rutas frecuentes</h3>
        ...
        {!loading && activeRoute.length > 0 && (
          ...
          >
            ...
            <button 
              ...
              className="p-3 bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/10 text-zinc-700 dark:text-zinc-300 rounded-xl hover:bg-zinc-100 dark:hover:bg-white/10 active:scale-95 transition-all flex items-center space-x-2 text-sm font-semibold shrink-0 self-stretch md:self-auto justify-center"
            >
            ...
            return (
              <div key={icao + "-node-" + idx} className="flex flex-col items-center space-y-3 relative z-10 w-24">
                ...
                <div className="text-center space-y-0.5">
                  ...
                  <div className="text-xs font-medium text-zinc-400 dark:text-zinc-500 leading-none">
      ...
      <div className="space-y-6">
        <h3 className="text-base font-bold text-zinc-950 dark:text-white px-1">Reportes detallados por estación</h3>
        
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-4">
            <RefreshCw className="w-8 h-8 animate-spin text-zinc-400" />
            <p className="text-sm font-medium text-zinc-400">Consultando meteorología y NOTAMs en vivo...</p>
          </div>
        ) : (
          <div className="space-y-4">
            {activeRoute.map((icao, idx) => {
              ...
              const stripKey = icao + "-" + idx;
              const isStripOpen = expandedStrip[stripKey];

              return (
                <div
                  key={icao + "-details-" + idx}
                  className={`bg-white dark:bg-[#111111] border border-zinc-200 dark:border-white/10 border-l-4 ${config.edge} rounded-2xl shadow-cal dark:shadow-none overflow-hidden transition-all duration-300 group`}
                >
                  {/* Flight strip header — always visible, ATC-strip style */}
                  <button
                    type="button"
                    onClick={() => toggleStrip(stripKey)}
                    className="w-full flex items-center gap-3 md:gap-5 px-4 md:px-6 py-3.5 md:py-4 text-left hover:bg-zinc-50 dark:hover:bg-white/[0.03] transition-colors"
                  >
                    <span className={`w-2 h-2 rounded-full ${config.dot} animate-pulse flex-shrink-0`} />
                    <div className="flex-1 min-w-0 grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-6 items-center">
                      ...
                      <div className="hidden md:block text-xs font-medium text-zinc-500 dark:text-zinc-400">
                        {w.windSpeed !== null && w.windSpeed > 0 ? `${w.windDir ?? "VRB"}° / ${w.windSpeed} KT` : "Calma"}
                      </div>
                      <div className="hidden md:block text-xs font-medium text-zinc-500 dark:text-zinc-400">
                        {w.temp !== null ? `${w.temp}°C` : "---"}
                      </div>
                    </div>
                    <ChevronDown className={`w-4 h-4 text-zinc-400 dark:text-zinc-500 flex-shrink-0 transition-transform ${isStripOpen ? "rotate-180" : ""}`} />
                  </button>
                  ...
                  {isStripOpen && (
                    <motion.div
                      ...
                      className="overflow-hidden border-t border-zinc-100 dark:border-white/5"
                    >
                    <div className="p-6 md:p-8 flex flex-col xl:flex-row gap-6 justify-between items-stretch">
                    ...
                    {madhel && (
                      <div className="bg-zinc-50 dark:bg-white/5 border border-zinc-100 dark:border-white/5 rounded-2xl p-4 space-y-3">
                        <div className="flex items-center justify-between border-b border-zinc-200/50 dark:border-white/5 pb-2">
                          <p className="text-xs font-medium text-zinc-400 dark:text-zinc-500 leading-none">Registro oficial MADHEL</p>
                  ...
                  <div className="flex-1 flex flex-col justify-between space-y-4 border-t xl:border-t-0 xl:border-l border-zinc-200 dark:border-white/10 pt-6 xl:pt-0 xl:pl-8">
                    ...
                    <div className="bg-zinc-900 dark:bg-black/40 border border-zinc-800 dark:border-white/5 rounded-2xl p-4 font-mono text-[10px] font-bold leading-relaxed text-zinc-300 dark:text-zinc-200 break-words shadow-inner flex-1 flex flex-col justify-between relative overflow-hidden group/box">
                      ...
                      <div className="flex items-center justify-between border-b border-zinc-800 dark:border-white/5 pb-2 mb-2">
                        <p className="text-[11px] font-sans font-semibold text-zinc-500 dark:text-zinc-400 leading-none">Código METAR registrado</p>
                    ...
                    <div className="space-y-2">
                      <button 
                        ...
                        className="w-full flex items-center justify-between text-sm font-medium text-zinc-400 dark:text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-colors py-1.5"
                      >
                            ...
                            >
                              <div className="bg-zinc-50 dark:bg-white/5 border border-zinc-200 dark:border-white/10 rounded-2xl p-4 space-y-4 text-xs">
                                ...
                                <div className="space-y-1.5">
                                  <p className="text-xs font-medium text-zinc-400 dark:text-zinc-500 leading-none">Pistas (runways)</p>
                                  <div className="space-y-1">
                                ...
                                <div className="space-y-1.5">
                                  <p className="text-xs font-medium text-zinc-400 dark:text-zinc-500 leading-none">Frecuencias de radio</p>
                                  <div className="space-y-1">
                                ...
                                {madhel.localization && (
                                  <div className="space-y-1">
                                    <p className="text-xs font-medium text-zinc-400 dark:text-zinc-500 leading-none">Ubicación de referencia</p>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                  <div className="space-y-1">
                                    <p className="text-xs font-medium text-zinc-400 dark:text-zinc-500 leading-none">Combustible</p>
                                  ...
                                  <div className="space-y-1">
                                    <p className="text-xs font-medium text-zinc-400 dark:text-zinc-500 leading-none">Teléfonos</p>
                                    <div className="space-y-1">
                                ...
                                {madhel.norms && (
                                  <div className="space-y-1">
                                    <p className="text-xs font-medium text-zinc-400 dark:text-zinc-500 leading-none">Normas particulares ANAC</p>
