...
import { AUDITORIA_HREF, esPestanaActiva, seccionDe } from "@/lib/secciones";

/**
 * Las pestañas de la sección en la que está el piloto.
 *
 * Es lo que hace posible que la barra tenga cuatro íconos y no nueve: Resumen,
 * Calendario y Auditoría siguen a un toque, pero desde adentro de la Bitácora; y
 * Aeropuertos, Clima y Herramientas desde "Preparar vuelo".
 *
 ...
 * nada.
 */
export default function SeccionTabs({ auditCount = 0 }: { auditCount?: number }) {
    ...
    >
      <div className="inline-flex items-center gap-1 p-1 rounded-2xl bg-zinc-100 dark:bg-white/5 border border-zinc-200 dark:border-white/10">
        {seccion.pestanas.map((pestana) => {
          const activa = esPestanaActiva(pestana, pathname);
          const hallazgos = pestana.href === AUDITORIA_HREF ? auditCount : 0;
          ...
          return (
            ...
            >
              ...
              {hallazgos > 0 && (
                <span
                  aria-label={`${hallazgos} ${hallazgos === 1 ? "hallazgo abierto" : "hallazgos abiertos"}`}
