...
6. **`06-plan-post-flightdeck.md`** — **El backlog vigente.** Escrito el 2026-08-01 después de implementar buena parte de `04` y `05` y de volver a comparar contra FlightDeck en vivo. Subdivide el trabajo restante en tareas chicas con id estable (`T0.1`, `T2.3`…) y, sobre todo, **corrige varias cosas que `04` y `05` dan por ciertas y ya no lo son** — entre ellas, que el "Nuevo Vuelo" de FlightDeck hoy es de dos columnas sin scroll con el desglose ANAC en un panel deslizante, no la columna vertical que describe `04`. Si hay discrepancia entre documentos, manda el `06`.
